...
protected:
	...
	int fill = 40; // Ekrana Yazmada Ýþlemlerinde Kullanýldý.
...
void Matrix<Type1>::inv() {
	Type1 _det = det();
	// Determinant sýfýr mý ve Matris Kare matris mi.
	if (_det != (Type1)0 && _det != (Type1)999999) {
		...
		// alt kýsmý ve üst kýsmý sýfýrlamak için 2 kez döndürüyoruz.
		for (int tur = 0; tur < 2; tur++) {
			for (minor = 0; minor < RC - 1; minor++) {
				zero = minor + 1;
				// Matrisin köþegen deðerleri 0 ise satýrlarý kendi içinde yer deðiþtiriyor
				...
				// köþegen deðerine göre çarpýlan deðer bulunup tüm satýra uygulandý.
				...
				// köþegen deðerlerin altýndaki tüm deðerler sýfýr(0) yapýldý.
			...
			// Matrisin sütünda ortasýna göre simetrisi alýndý
			...
			// Matrisin satýrda ortasýna göre simetrisi alýndý
			length = RC;
			for (unsigned int y = 0; y < RC / 2; y++) {
				length--;
				for (unsigned int z = 0; z < RC; z++) {
...
Type1 Matrix<Type1>::det() {
	...
	if (this->row == this->col) {
		...
		// Matrisin Köþegen Deðerlerinin altýnda kalan kýsýmlarý sýfýr(0) yapýldý.
		// Eðer köþegen deðeri 0 olan varsa baþka bir satýrla deðiþtirilip iþleme devam edildi.
		for (minor = 0; minor < RC - 1; minor++) {
		...
		// Matrisin Köþegen deðerleri Çarpýlarak Determinant bulundu.
		for (unsigned int i = 0; i < RC; i++) {
		...
		// Fonksiyon Ýçinde Oluþturulan Yardýmcý matris ve Dizi silindi.
...
/* **************** Image.h **************** */
//#include "Matrix.h"
// RGB türünden Struct Yapýsý Oluþturuldu.


// Image Sýnýfý Matrix Base Class ýndan ve RGB Struct Yapýsýndan Miras Aldý.
template <class Type2>
class Image : public Matrix<Type2> {
public:
	struct RGB {
		// r(red) g(green) b(blue) a(opacity) deðeri içindir.
		int r, g, b;
		double a;
	};
...
private:
	...
	int GrayBW = 0;
	streampos size;
	// Dosya Okuma ve Yazma Ýþlemleri Ýçin Yardýmcý Arrayler Tanýmlandý.
...
int main() {
	...
	//Integer Default 10x10 0 Deðerli Matris
	Matrix<int>* m1 = new Matrix<int>();
	//Integer 5x5 3 Deðerli Matris			
	...
	//Integer 6x6 0-255 Random Deðerli Matris
	Matrix<double>* m5 = new Matrix<double>(4, 4, 'r');
	//Integer 6x4 0-255 Random Deðerli Matris	
	Matrix<double>* m6 = new Matrix<double>(6, 4, 'r');
	//Char 4x6 '*' Deðerli Matris	
	...
	//t3->setRowNames(rs, 5);
	//cout << "4. Satir, C :" << t3->itemAt("4. Satýr", "C") << endl;
	//string cs[] = { "0. Sutun",  "1. Sutun", "2. Sutun", "3. Sutun", "4. Sutun" };
	//t3->setColNames(cs, 5);
	//cout << "4. Satir, 2. Sutun :" << t3->itemAt("4. Satýr", "2. Sutun") << endl << endl;
	//t3->print();
